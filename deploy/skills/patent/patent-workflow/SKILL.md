...
- 交底书模板（章节结构）：`templates/技术交底书模板-发明新型.docx`（自备，见 `templates/README.md`），章节 **零~八 + 3 个无编号节**（商业价值/侵权证据可获得性/标准进展情况/其他技术资料）；旧「六、价值体现」4 维度已取消；新「三」=现有技术方案（不含缺点），「四」=缺点+技术问题，「六」=关键点和欲保护点，「七」=技术优点，「八」=规避思考；无保密页眉（页脚页码）
- **输出格式基准（2026-09-07 用户定稿）**：`templates/技术交底书模板-发明新型-用户定稿.docx`（自备）——黑体标题 16/15/15/14pt、宋体/Times 正文小四 1.25 倍行距首行缩进 2 字符、封面仿宋蓝色、表格宋体 10pt、标题 Word 自动编号（一、~八、+ 子节 N.M）；md2docx 已全量按此输出（旧官方模板仅留档）
- 检索报告模板：`templates/检索报告模板（修订版）.docx`（自备）；二→四跳号保持原样
...
- 模板：均需自备放入 `templates/`（`*.docx` 不入库，见 `templates/README.md`）——`技术交底书模板-发明新型-用户定稿.docx`（★输出格式基准）、`技术交底书模板-发明新型.docx`（官方原版留档）、`检索报告模板（修订版）.docx`（检索报告，本次格式更新未含）
...
- **公开仓库约定（2026-09-10 起）**：仓库已转公开——禁止把公司名/姓名/联系方式/内部代号写进仓库；模板 `*.docx` 不入库（见 `templates/README.md`）；本地实跑版（skill/SOUL/模板）可能含真实封面信息、仅限本地——**任何内容同步进仓库前先跑 `python deploy/privacy_check.py`**。

- 产出命名：`<发明名称>-交底书VN.docx` / `<发明名称>-检索报告VN.docx`（N 从 1 起，修改即 V+1 副本）；封面字段（申报单位/类型/联系人）按案件填写
