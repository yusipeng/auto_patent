...
把论文预印本 → 公司专利交底书 docx + 检索报告 docx 的自动化流水线（8 个 Hermes Bot 专人专项协作）。
...
- **格式**：以用户定稿格式基准输出（md2docx 已内置）；公式为 Word 原生 OMML（`$…$` 行内、```latex 块）。
